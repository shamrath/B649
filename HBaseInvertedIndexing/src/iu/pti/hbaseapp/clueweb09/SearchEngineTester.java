...
public class SearchEngineTester {
    ...
    public static void searchKeyword(String keyword) throws Exception {
        ...
        int pageCount = 0;
        if (indexRow != null && indexRow.list() != null && !indexRow.list().isEmpty()) {
            for (KeyValue kv : indexRow.list()) {
                ...
                pageDocId = Bytes.toString(split.getQualifier());
                freq = Bytes.toInt(split.getValue());
                // Step 2: get the URI of the page from clueWeb09DataTable
                Get gData = new Get(pageDocId.getBytes());
                Result dataRow = dataTable.get(gData);
                if (dataRow != null) {
                    for (KeyValue dKv : dataRow.list()) {
                        if (Bytes.toString(dKv.getFamily()).equals(Constants.CF_DETAILS)) {
                            if (Bytes.toString(dKv.getQualifier()).equals(Constants.QUALIFIER_URI)) {
                                pageUri = Bytes.toString(dKv.getValue());
                            }
                        }
                    }
                }
                // Step 3: get the page rank value of this page from clueWeb09PageRankTable
                Get gRank = new Get(pageDocId.getBytes());
                Result prRow = prTable.get(gRank);
                if (prRow != null) {
                    for (KeyValue prTableKv : prRow.list()) {
                        if (Bytes.toString(prTableKv.getFamily()).equals(Constants.CF_PAGERANK)) {
                            pageRank = Bytes.toFloat(prTableKv.getQualifier());
                        }
                    }
                }
